...
namespace UnityEngine.XR.ARFoundation.Samples
{
    public class BoneController : MonoBehaviour
    {
        ...
        ARHumanBody m_HumanBody;

        void Start()
        {
            // Grab the AR Human Body component attached to this same Prefab
            m_HumanBody = GetComponent<ARHumanBody>();
            
            // Map the bones once when the bot spawns
            InitializeSkeletonJoints();
        }

        void Update()
        {
            // Apply the tracking math every single frame
            if (m_HumanBody != null)
            {
                ApplyBodyPose(m_HumanBody);
            }
        }
